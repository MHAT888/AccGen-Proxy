proxies={"https":"http://pekig71768_maxsad_com:ya123456@la.residential.rayobyte.com:8000"}
...
def request_code(mhat ,email: str):
	...
	req = requests.post(f"https://service.aminoapps.com/api/v1/g/s/auth/request-security-validation", data = data, headers = headers, proxies=proxies)
...
def check_code(mhat, code:str,email: str):
    ...
    headers["NDC-MSG-SIG"] = NDCMSGSIG(mhat=NDCMSGSIG,data = data)
    headers["NDCDEVICEID"] = deviceID()
    req = requests.post('https://service.aminoapps.com/api/v1/g/s/auth/check-security-validation', headers=headers, data=data, proxies=proxies)
...
def register(mhat, password: str, email: str, code: str, nickname: str):
    ...
    headers["NDC-MSG-SIG"] = NDCMSGSIG(mhat=NDCMSGSIG,data = data)
    headers["NDCDEVICEID"] = deviceID()
    req = requests.post('https://service.aminoapps.com/api/v1/g/s/auth/register', headers=headers, data=data, proxies=proxies)
    if req.status_code==200:
...
def login(mhat ,email: str, password: str):
	...
	headers["NDC-MSG-SIG"] = NDCMSGSIG(mhat=NDCMSGSIG,data = data)
	req = requests.post(f"https://service.aminoapps.com/api/v1/g/s/auth/login", data = data, headers = headers, proxies=proxies)
	if req.status_code!= 200:
		sleep(0.1)
		main()
...
def get_cid(mhat):
	...
	return requests.get(f"https://service.aminoapps.com/api/v1/g/s/link-resolution?q={CommunityLink}",headers = headers, proxies=proxies).json()
...
def join_community(mhat):
	...
	headers["NDC-MSG-SIG"] = NDCMSGSIG(mhat=NDCMSGSIG,data = data)
	headers["NDCAUTH"] = f"sid={login.sid}"
	req = requests.post(f"https://service.aminoapps.com/api/v1/x{ComId}/s/community/join", data = data, headers = headers, proxies=proxies)
	if req.status_code!=200:
		print(req.json()["api:message"])
